    ...
    for m in [j-1,j,j+1]: 
      if l>=0 and m>=0 and l<len(maze) and m<len(maze[0]) and not((l,m)==(i,j)) and maze[l][m]!=1: 
        if (l,m) not in visited: pos_moves.append((l,m)) 
          return pos_moves def search(src,target,visited,d): 
          visited.append(src) 
          if src==target: 
            return visited pos_moves=possible_moves(src,visited) 
          if(pos_moves==[]): 
            return False scores=[h(x,target)+d for x in pos_moves] min_score=min(scores) selected_moves=[] for i in range(len(pos_moves)): 
